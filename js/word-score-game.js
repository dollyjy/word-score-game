...
function findWordToUse() {
    ...
    if ($("#word-history-list li").length == 0) {
        return;
    }

    var highestScoreWord = $("#word-history-list li:first-child");
    $("#word-history-list li").each(function () {
        if ($(this).find("span").text() > $(highestScoreWord).find("span").text()) {
            highestScoreWord = this;
        }
    });
    $(highestScoreWord).css("color", "red");
}
...
function takeOutUsedLetters() {
    ...
    for (ii = 0; ii < YOUR_HAND.length; ii++) {
        if (YOUR_HAND[ii].used) {
            ...
            currentWordScore += YOUR_HAND[ii].pointsWhenLettersUsed;
    ...
    $("#word-history-list li:last-child").append("<span style='margin: 10px'>" + currentWordScore + "</span>");
...
function retireHand() {
    ...
    if (BAG_OF_LETTERS.length == 0) {
        alert("BAG_OF_LETTERS has no letters")
    } else {
        displayHand();
    }
}
